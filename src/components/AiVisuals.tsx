...
export default function AiVisuals() {
  ...
  return (
    <section aria-label="AI Visuals gallery" role="region">
      <div ref={containerRef} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', padding: '2rem' }}>
        {visuals.map((src, i) => (
          <motion.figure key={i} initial={{ opacity: 0, y: 20, scale: 0.98 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.8, ease: 'easeOut' }} style={{ overflow: 'hidden', borderRadius: 12 }}>
            <Image src={'/' + src} alt={getAltText(src)} width={800} height={600} style={{ width: '100%', height: 'auto', display: 'block' }} loading="lazy" />
            <figcaption style={{ color: '#9aa7b8', fontSize: '0.85rem', padding: '0.5rem' }}>{getCaption(src)}</figcaption>
          </motion.figure>
        ))}
      </div>
    </section>
